...
* **순서가 어긋난 도착**: 메시지는 만들어진 순서대로 도착하지 않습니다. 나중에 만들어진 메시지가 먼저 도착하는 일이 흔합니다.
...
| 데이터 시간 필드 | - | string | 윈도우 구간을 나눌 기준이 되는 시간 필드명을 반드시 입력합니다.<br/>값을 시각으로 해석할 수 없는 메시지는 집계에서 제외합니다. | 스키마 정의 시 드롭다운 제공 |
...
한국 시각 자정을 기준으로 하루를 집계하려면 상위 [Date](#filter-date) 노드에서 데이터 시간 필드 값에 9시간을 더한 필드를 만들어, 그 필드를 데이터 시간 필드로 지정하세요. 이때 출력 메시지의 `window_start`와 `window_end`도 9시간 더해진 값으로 나오므로, 결과를 해석할 때 이 점을 고려해야 합니다.
...
5. 데이터 시간 필드 값이 현재 시각보다 과거인지 확인합니다. 과거 데이터를 다시 넣는 경우 윈도우 경계도 그 과거 시각을 따릅니다.
