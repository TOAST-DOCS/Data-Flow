...
| 인스턴스 타입 | 권장 그룹 키 종류 수 | 메모리 점검 기준 |
|---|---|---|
| c1m2 | 10만 개 이하 | 600MB |
| c2m4 | 40만 개 이하 | 1.3GB |

* 메모리 사용량이 점검 기준을 계속 초과하면 그룹 키 종류 수를 점검하세요. 메모리 사용량은 **Data & Analytics > DataFlow > 모니터링**의 V2-TASK 메모리 사용량에서 확인할 수 있습니다.
* 그룹 키 종류 수가 한도를 초과하면 메모리 부족으로 플로우가 오류 상태에 빠지며 자동으로 복구되지 않습니다. 플로우를 중지한 후 그룹 키를 조정하고 다시 실행하세요.
