...
| 오프셋 커밋 주기        | `5000`                            | number           | 컨슈머 그룹의 오프셋을 갱신할 주기(ms)를 입력합니다.                                                 | [Kafka 공식 문서](https://kafka.apache.org/39/configuration/consumer-configs/)의 `auto.commit.internal.ms` 속성 참고                                                                                                                                                                                                                                          |
...
| 서버 재연결 주기        | `100`                             | number           | 브로커 서버에 연결이 실패했을 때 재시도할 주기(ms)를 입력합니다.                                          | [Kafka 공식 문서](https://kafka.apache.org/39/configuration/consumer-configs/)의 `reconnect.backoff.ms` 속성 참고                                                                                                                                                                                                                                             |
...
| 속성명      | 기본값         | 자료형     | 설명                                              | 비고                                               |
|----------|-------------|---------|-------------------------------------------------|--------------------------------------------------|
| 소스 필드    | -           | string  | 토큰화 할 소스 필드명을 입력합니다.                            |                                                  |
| 저장할 필드   | -           | string  | 토큰화 결과를 저장할 필드명을 입력합니다.                         |                                                  |
| 정규식      | `\s+`       | string  | 토큰화에 사용될 정규식을 입력합니다.                            |                                                  |
| 모드       | `SEPARATOR` | enum    | 토큰화 모드를 입력합니다.                                  | SEPERATOR: 정규식을 구분자로 사용<br>MATCH: 정규식을 토큰 매칭에 사용 |
...
| 초기 시드 | - | number | 난수 생성 시 사용할 초기 시드를 입력합니다. 초기 시드가 같고 입력 메시지가 동일하다면 결과는 동일합니다. |  |
