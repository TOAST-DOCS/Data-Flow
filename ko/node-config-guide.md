...
### 노드 설명 { #filter-aggregate-node-description }

* 메시지를 일정한 길이의 시간 구간(윈도우)으로 나눈 다음, 그룹별로 집계하는 노드입니다.
* 윈도우는 서로 겹치지 않는 같은 길이의 구간으로 나뉩니다. 한 메시지는 하나의 윈도우에만 속합니다.
* 윈도우 구간은 데이터 시간 필드의 값을 기준으로 나눕니다. 노드가 메시지를 처리한 시각은 사용하지 않습니다.
* 윈도우가 닫히면 그룹마다 집계 결과 메시지를 1건 출력합니다. 원본 메시지는 출력하지 않습니다.
* Aggregate 노드는 입력 연결을 하나만 가질 수 있습니다. 입력 연결이 2개 이상이면 플로우를 저장할 수 없습니다.
* 집계 대상을 좁히려면 상위에 [Branch > IF](#branch-if) 노드를 연결해 조건에 맞는 메시지만 보내세요. 집계 결과 중 일부만 다음 노드로 보내려면 하위에 [Branch > IF](#branch-if) 노드를 연결하세요.

노드를 설정하기 전에 다음 개념을 확인하세요.

* **윈도우**: 집계 대상을 묶는 시간 구간입니다. 구간에 해당하는 메시지가 모두 도착했다고 판단하면 윈도우를 닫고 결과를 출력합니다. 닫힌 윈도우는 다시 열리지 않습니다.
* **순서가 어긋난 도착**: 메시지가 데이터 시간 필드 값의 순서대로 도착한다는 보장은 없습니다. 메시지를 만든 쪽의 전송 지연, 파티션이 여러 개인 소스, 병렬 처리 과정 등 여러 원인으로 나중에 만들어진 메시지가 먼저 도착할 수 있습니다.
* **워터마크 지연 시간**: 그래서 구간이 끝나자마자 윈도우를 닫지 않고 더 기다립니다. 얼마나 기다릴지를 정하는 값입니다.
* **지연 허용 시간**: 윈도우가 닫힌 뒤에 도착한 메시지까지 반영하려면 이 값을 설정합니다.

두 지연 시간의 차이는 [워터마크 지연 시간과 지연 허용 시간](#filter-aggregate-watermark)에서 설명합니다.

<a id="filter-aggregate-property-description"></a>
### 속성 설명 { #filter-aggregate-property-description }

| 속성명 | 기본값 | 자료형 | 설명 | 비고 |
|---|---|---|---|---|
| 그룹 키 | - | array of strings | 그룹으로 묶을 필드명을 입력합니다.<br/>입력하지 않으면 전체 메시지를 하나의 그룹으로 집계합니다. | 스키마 정의 시 드롭다운 제공 |
| 데이터 시간 필드 | - | string | 윈도우 구간을 나눌 기준이 되는 시간 필드명을 입력합니다.<br/>값을 시각으로 해석할 수 없는 메시지는 집계에서 제외합니다. | 스키마 정의 시 드롭다운 제공 |
| 윈도우 크기 | 60 | number | 윈도우 하나의 길이를 초 단위로 입력합니다. |  |
| 워터마크 지연 시간 | 0 | number | 윈도우 구간이 끝난 뒤에도 늦게 도착하는 메시지를 더 기다릴 시간을 초 단위로 입력합니다.<br/>값이 클수록 늦게 온 메시지를 더 많이 반영하지만 결과가 그만큼 늦게 나옵니다. |  |
| 지연 허용 시간 | 0 | number | 윈도우가 닫힌 뒤에도 집계 상태를 유지할 시간을 초 단위로 입력합니다. |  |
| 집계 항목 | - | hash | 윈도우마다 계산할 집계 목록을 최소 한 행 이상 입력합니다. | `집계 항목 입력 방법` 참고 |

<a id="filter-aggregate-property-description-how-to-enter-aggregations"></a>
#### 집계 항목 입력 방법

* **집계 추가**를 클릭해 행을 추가하고, 행마다 집계할 필드, 연산, 출력 필드명을 입력합니다.
* 집계 항목은 최소 한 행 이상 입력해야 합니다.
* 출력 필드명은 서로 중복할 수 없으며, 그룹 키 필드명이나 `window_start`, `window_end`와 같을 수 없습니다.

| 연산 | 집계할 필드 | 설명 |
|---|---|---|
| COUNT | 비워 둡니다. | 윈도우에 포함된 전체 메시지 수를 셉니다. 한 행만 추가할 수 있습니다. |
| SUM | 반드시 지정합니다. | 집계할 필드 값의 합을 계산합니다. |
| AVG | 반드시 지정합니다. | 집계할 필드 값의 평균을 계산합니다. |

* SUM, AVG는 집계할 필드 값이 숫자인 메시지만 집계합니다. 값이 없거나 숫자로 해석할 수 없는 메시지는 계산에서 제외합니다.
* 윈도우 안에 숫자로 해석할 수 있는 값이 하나도 없으면 SUM, AVG 결과는 `null`입니다.

<a id="filter-aggregate-window-boundary"></a>
### 윈도우 구간 { #filter-aggregate-window-boundary }

윈도우 경계는 플로우 실행 시각이나 첫 메시지의 시각이 아니라, 1970년 1월 1일 00:00:00 UTC를 기점으로 윈도우 크기의 배수가 되는 시각에 맞춰 나눕니다. 예를 들어 윈도우 크기가 60초이면 매분 0초, 3600초이면 매시 정각이 경계입니다.

시각은 UTC 기준입니다. 따라서 윈도우 크기를 86400(1일)으로 설정하면 하루의 경계는 한국 시각 기준 오전 9시입니다.

집계 윈도우는 UTC 기준으로 정렬됩니다. 하루 단위 윈도우의 경계는 UTC 자정, 즉 한국 시각 오전 9시이며, 현재는 하루의 경계를 한국 시각 자정에 맞추는 옵션을 제공하지 않습니다. window_start와 window_end는 UTC 기준 epoch 밀리초로 출력됩니다

<a id="filter-aggregate-data-time-field"></a>
### 데이터 시간 필드 { #filter-aggregate-data-time-field }

데이터 시간 필드에 지정한 필드의 값은 다음 형식을 지원합니다.

| 형식 | 예 |
|---|---|
| epoch 밀리초(숫자 또는 문자열) | `1785730000000`, `"1785730000000"` |
| ISO 8601 UTC 문자열 | `2026-08-19T09:30:00Z` |
| 오프셋을 포함한 ISO 8601 문자열 | `2026-08-19T18:30:00+09:00` |

* 숫자 값은 항상 밀리초 단위로 해석합니다. 초 단위 epoch 값을 입력하면 1970년에 가까운 시각으로 해석되어 의도한 윈도우로 나뉘지 않습니다.
* 오프셋이 없는 문자열은 위 형식에 해당하지 않으므로 그 값을 가진 메시지는 집계에서 제외합니다. 예를 들어 `2026-08-19 18:30:00`, `2026-08-19T18:30:00`은 모두 사용할 수 없습니다.
* 값을 그대로 사용할 수 없다면 상위에 [Date](#filter-date) 노드를 연결해 형식을 변환하세요.

<a id="filter-aggregate-watermark"></a>
### 워터마크 지연 시간과 지연 허용 시간 { #filter-aggregate-watermark }

두 속성 모두 늦게 도착하는 메시지를 다루지만 작동하는 시점과 결과가 다릅니다.

| 항목 | 워터마크 지연 시간 | 지연 허용 시간 |
|---|---|---|
| 작동 시점 | 윈도우를 닫기 전 | 윈도우가 닫힌 뒤 |
| 결과 출력 | 윈도우당 1건 | 늦은 메시지가 올 때마다 1건씩 추가 출력 |
| 결과가 나오는 시점 | 설정한 만큼 늦어짐 | 늦어지지 않음 |
| 메모리 | 영향 없음 | 닫힌 윈도우도 유지하므로 증가 |

결과를 한 번만 받고 싶다면 지연 허용 시간을 0으로 두고 워터마크 지연 시간만 조정하세요. 결과를 빨리 받되 늦게 온 데이터도 놓치고 싶지 않고 중복 결과를 하위에서 처리할 수 있다면 지연 허용 시간을 사용하세요.

두 속성 모두 실제 시각이 아니라 데이터 시간 필드의 값을 기준으로 시간을 잽니다.

Aggregate 노드는 메시지가 도착한 순서가 아니라 데이터 시간 필드의 값을 기준으로 윈도우를 닫습니다. 윈도우를 닫는 시점은 다음 순서로 정합니다.

1. 지금까지 도착한 메시지의 데이터 시간 중 가장 큰 값을 기준 시각으로 삼습니다.
2. 기준 시각에서 워터마크 지연 시간을 뺍니다. 이 값이 "이 시각 이전의 메시지는 모두 도착했다"고 판단하는 기준입니다.
3. 이 값이 윈도우 종료 시각에 도달하면 해당 윈도우를 닫고 결과를 출력합니다.

예를 들어 윈도우 크기가 10초, 워터마크 지연 시간이 5초일 때 데이터 시간이 15초인 메시지가 도착하면 15초 - 5초 = 10초가 되어, 0초 이상 10초 미만 윈도우가 닫힙니다.

* 워터마크 지연 시간이 클수록 순서가 어긋나 도착한 메시지를 더 많이 반영하지만, 그만큼 결과 출력도 늦어집니다.
* (Apache) Kafka나 (NHN Cloud) EasyQueue는 데이터를 여러 갈래(파티션)로 나눠 저장하는데, 갈래마다 읽는 속도가 달라 나중에 만들어진 메시지가 먼저 도착하는 일이 자주 발생합니다. 이런 소스를 사용한다면 워터마크 지연 시간을 넉넉하게 설정하세요.
* 윈도우가 닫힌 뒤에 도착한 메시지는 버립니다. 지연 허용 시간을 설정하면 그 시간 동안 집계 상태를 유지하여 늦게 도착한 메시지도 반영합니다.
* 단, 지연 허용 시간을 0보다 크게 설정하면 늦게 도착한 메시지마다 해당 윈도우의 결과를 다시 출력합니다. 따라서 같은 `window_start` 값을 가진 결과 메시지가 여러 건 나오며, 그중 마지막에 출력한 메시지가 최종 집계 값입니다.
    * 이전에 출력한 결과를 갱신하지는 않습니다. 하위 노드나 Sink에서 결과를 다시 집계하거나 중복으로 저장하지 않도록 주의하세요.
    * 중복 출력을 원하지 않는다면 지연 허용 시간을 0으로 두고, 워터마크 지연 시간을 늘려 늦게 도착하는 메시지를 윈도우가 닫히기 전에 반영하세요.

<a id="filter-aggregate-output-event"></a>
### 출력 메시지 구성 { #filter-aggregate-output-event }

출력 메시지는 다음 필드로만 구성합니다. 그룹 키로 지정하지 않은 입력 필드는 모두 사라집니다.

| 필드 | 데이터 타입 | 설명 |
|---|---|---|
| 그룹 키로 지정한 필드 | 입력 메시지와 동일 | 그룹 키 값을 그대로 유지합니다. |
| window_start | Long | 윈도우 시작 시각(epoch 밀리초). 윈도우 구간에 포함됩니다. |
| window_end | Long | 윈도우 종료 시각(epoch 밀리초). 윈도우 구간에 포함되지 않습니다. |
| 집계 항목의 출력 필드명 | COUNT는 Long, SUM과 AVG는 Double | 집계 항목마다 결과 값을 하나씩 담습니다. |

<a id="filter-aggregate-example"></a>
### 집계 예제 { #filter-aggregate-example }
...
| 연산 | 집계할 필드 | 출력 필드명 |
|---|---|---|
| COUNT | 비워 둠 | `message_count` |
| SUM | `cnt` | `sum_cnt` |
| AVG | `cnt` | `avg_cnt` |

계산 과정을 쉽게 보여 주기 위해 `event_ts`에 작은 epoch 밀리초 값을 사용했습니다. 실제 데이터에서는 `1785730001000`처럼 13자리 값이 들어오고, `window_start`와 `window_end`도 같은 자릿수로 출력됩니다.
...
`event_ts`가 5000인 메시지는 9000인 메시지보다 늦게 도착했지만, 0 이상 10000 미만 윈도우가 아직 열려 있으므로 집계에 포함됩니다. 워터마크 지연 시간을 0으로 설정했다면 이 윈도우는 `event_ts`가 9000인 메시지가 도착한 시점에 이미 닫혀 5000인 메시지를 놓쳤을 것입니다.

`event_ts`가 15000인 메시지가 도착하면 15000 - 5000 = 10000이 되어 0 이상 10000 미만 윈도우가 닫히고, 그룹마다 1건씩 총 2건을 출력합니다.
...
마지막으로 도착한 `event_ts`가 7000인 메시지는 이미 닫힌 윈도우에 속하므로 버립니다. 지연 허용 시간을 5로 설정했다면 이 메시지를 반영한 `window_start`가 0인 결과를 한 번 더 출력합니다.

`event_ts`가 12000, 15000인 메시지가 속한 10000 이상 20000 미만 윈도우는 아직 닫히지 않았습니다. 이 윈도우는 `event_ts`가 25000 이상인 메시지가 도착해야 닫히며, 메시지가 더 들어오지 않으면 결과를 출력하지 않습니다.

<a id="filter-aggregate-notes"></a>
### 주의 사항 { #filter-aggregate-notes }

<a id="filter-aggregate-notes-memory"></a>
#### 메모리 사용량

Aggregate 노드의 메모리 사용량은 그룹 키 값의 종류 수에 비례합니다. 윈도우 크기나 처리하는 메시지 양은 메모리 사용량에 영향을 주지 않습니다.

* 그룹 키 값의 종류 수는 인스턴스 타입에 따라 다음 값 이하를 권장합니다. 그룹 키를 여러 개 지정하면 종류 수는 각 키 값 조합의 수입니다. 예를 들어 지역이 10종, 장비가 1만 종이면 종류 수는 10만 개입니다.

| 인스턴스 타입 | 권장 그룹 키 종류 수 | 기본 메모리 사용량 | 메모리 점검 기준 |
|---|---|---|---|
| c1m2 | 10만 개 이하 | 450~500MB | 600MB |
| c2m4 | 40만 개 이하 | 900~950MB | 1.3GB |

플로우가 메시지를 처리하지 않는 동안에도 기본 메모리 사용량만큼은 계속 사용합니다. 이는 정상이며 그룹 키 값의 종류 수와 무관합니다.

인스턴스 타입이 클수록 플로우에 할당되는 메모리가 커지고, 그만큼 사용을 마친 메모리를 정리하는 시점도 늦어집니다. 따라서 기본 메모리 사용량은 집계에 실제로 쓰이는 양이 아니라, 아직 정리하지 않은 메모리까지 포함한 값입니다. 인스턴스 타입이 클수록 기본 메모리 사용량도 높아지는 이유입니다.

* 메모리 사용량은 **Data & Analytics > DataFlow > 모니터링**의 V2-TASK 메모리 사용량에서 확인할 수 있습니다.
* 메모리 사용량이 일시적으로 점검 기준을 넘는 것은 정상입니다. 30분 이상 점검 기준을 넘은 상태가 이어지면 그룹 키 값의 종류 수를 점검하세요.
* 그룹 키 종류 수가 한도를 초과하면 메모리 부족으로 플로우가 오류 상태에 빠지며 자동으로 복구되지 않습니다. 플로우를 종료한 후 그룹 키를 조정하고 다시 실행하세요.
* 그룹 키에는 값의 종류가 한정된 필드를 지정하세요. 사용자 ID, 세션 ID, 요청 ID, IP 주소처럼 시간이 지날수록 새 값이 계속 생기는 필드를 그룹 키로 지정하면 종류 수에 상한이 없어 결국 메모리가 부족해집니다.
* 더 많은 그룹 키 종류 수가 필요하면 인스턴스 타입 상향을 검토하세요.
* 지연 허용 시간을 0보다 크게 설정하면 닫힌 윈도우의 상태도 지연 허용 시간 동안 유지되어 메모리 사용량이 최대 (1 + 지연 허용 시간 ÷ 윈도우 크기)배까지 늘어납니다. 그 배수만큼 그룹 키 종류 수를 줄여 사용하세요.

<a id="filter-aggregate-notes-behavior"></a>
#### 알려진 동작

* 윈도우가 닫혀 결과를 출력하면 해당 윈도우가 사용하던 메모리는 반환됩니다. 메모리 사용량을 결정하는 것은 누적 처리량이 아니라 하나의 윈도우 안에 동시에 존재하는 그룹 키 종류 수입니다.
* 플로우를 종료하면 진행 중이던 윈도우의 집계 상태는 유지되지 않습니다. 다시 실행하면 해당 윈도우는 빈 상태에서 다시 집계합니다.
* 메시지가 없는 윈도우는 결과를 출력하지 않습니다.
* 플로우가 실행 중인 동안 마지막 윈도우는 다음 윈도우의 메시지가 도착해야 닫힙니다. 메시지가 더 이상 들어오지 않으면 마지막 윈도우의 결과는 출력하지 않습니다.
* 지연 허용 시간을 0보다 크게 설정하면 같은 윈도우의 결과를 여러 번 출력합니다. Object Storage나 Kafka처럼 이미 적재한 데이터를 갱신하지 않는 Sink에서는 같은 윈도우의 결과가 여러 건 쌓이므로, 조회하는 쪽에서 `window_start` 값이 같은 결과 중 마지막 1건만 사용해야 합니다.
* 윈도우가 닫힌 뒤에 도착해 집계하지 못한 메시지와 데이터 시간 필드를 해석할 수 없어 제외한 메시지는 별도로 출력하지 않습니다.
* SUM과 AVG는 실수 연산으로 계산하므로, 자릿수가 큰 값을 집계하면 오차가 생길 수 있습니다.

<a id="filter-aggregate-notes-termination"></a>
#### 플로우 종료 시 마지막 윈도우

플로우가 종료될 때 아직 구간이 끝나지 않은 윈도우는 종료 방식에 따라 다르게 처리합니다.

| 종료 방식 | 진행 중이던 윈도우 |
|---|---|
| 플로우 종료 | 결과를 출력하지 않고 사라집니다. |
| 플로우 드레이닝 후 종료 | 구간이 끝나지 않았어도 그때까지 집계한 결과를 출력합니다. |
| BATCH 모드에서 데이터를 모두 처리한 후 자동 종료 | 구간이 끝나지 않았어도 그때까지 집계한 결과를 출력합니다. |

이렇게 출력한 결과는 **구간 전체를 집계한 결과와 형식이 같아 구별할 수 없습니다.** `window_start`와 `window_end`도 원래 구간 값이 그대로 들어가므로, 구간 전체를 집계한 값으로 오해할 수 있습니다.

* 종료 직후에 출력된 마지막 윈도우의 결과는 부분 집계일 수 있습니다. 집계 결과를 비교하거나 합산할 때 이 점을 고려하세요.
* BATCH 모드로 정해진 기간의 데이터를 집계할 때는 마지막 윈도우가 항상 부분 집계가 될 수 있습니다. 처리할 데이터의 시간 범위를 윈도우 크기의 배수로 맞추면 이 문제를 피할 수 있습니다.
...
1. 데이터 시간 필드 값의 형식을 확인합니다. 지원하지 않는 형식이면 해당 메시지는 집계에서 제외되며 별도의 오류를 출력하지 않습니다.
2. 초 단위 epoch 값을 사용하고 있지 않은지 확인합니다. 초 단위 값은 1970년에 가까운 시각으로 해석되어 의도한 윈도우로 나뉘지 않습니다.
3. 윈도우가 아직 닫히지 않았을 수 있습니다. 윈도우는 다음 윈도우에 속하는 메시지가 도착해야 닫힙니다. 데이터가 드물게 들어온다면 윈도우 크기를 줄이거나 데이터가 더 쌓일 때까지 기다려 보세요.
4. 워터마크 지연 시간이 지나치게 크지 않은지 확인합니다. 이 값만큼 결과 출력이 늦어집니다.
5. 과거 데이터를 다시 넣고 있는지 확인합니다. 윈도우는 데이터 시간 필드 값 기준으로 나뉘므로, 결과의 window_start와 window_end도 그 과거 시각으로 출력됩니다. 현재 시각 근처에서 결과를 찾고 있지 않은지 확인하세요. 또한 플로우가 이미 더 최신 시각의 데이터를 처리했다면, 과거 데이터는 이미 닫힌 윈도우에 속해 집계되지 않고 버려집니다.
