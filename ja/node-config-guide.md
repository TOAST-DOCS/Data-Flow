...
* ウィンドウの区間は、データ時間フィールドの値を基準に分割します。ノードがメッセージを処理した時刻は使用しません。
...
* **順序が乱れた到着**: メッセージがデータ時間フィールド値の順序どおりに到着するとは限りません。メッセージを生成した側の送信遅延、複数パーティションのソース、並列処理など、さまざまな原因により、後から生成されたメッセージが先に到着する場合があります。
...
| プロパティ名 | デフォルト値 | データ型 | 説明 | 備考 |
|---|---|---|---|---|
| グループキー | - | array of strings | グループでまとめるフィールド名を入力します。<br/>入力しない場合は、すべてのメッセージを1つのグループとして集計します。 | スキーマ定義時にドロップダウンを提供 |
| データ時間フィールド | - | string | ウィンドウ区間を分割する基準となる時間フィールド名を入力します。<br/>値を時刻として解釈できないメッセージは集計から除外されます。 | スキーマ定義時にドロップダウンを提供 |
...
* ウィンドウが閉じた後に到着して集計できなかったメッセージと、データ時間フィールドを解析できないため除外されたメッセージは、別途出力されません。
