...
| パス方式リクエスト     | `false`                        | boolean | V2       | パス方式リクエストを使用するかどうかを決定します。                                                                            |                                                                                                                                                                                                              |

!!! danger "注意"
    * (Amazon) S3ノードを利用してNHN Cloud Object Storageに接続する場合、以下のように属性設定を行う必要があります。
    * エンジンタイプがV1の場合
...
| 属性名 | デフォルト値 | 資料型 | 対応エンジンタイプ | 説明 | 備考 |
|------------------|------------------------------------------------------------|------------------|----------|----------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ブローカーサーバーリスト | `localhost:9092` | string | V1 | Kafkaブローカーサーバーを入力します。サーバーが複数台の場合、カンマ(`,`)で区切ります。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`bootstrap.servers`属性参照 <br/>例: 10.100.1.1:9092,10.100.1.2:9092 |
| コンシューマーグループID | `dataflow` | string | V1 | Kafka Consumer Groupを識別するIDを入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`group.id`属性参照 |
...
| 内部トピック除外可否 | `true` | boolean | V1 | __consumer_offsetsのような内部トピックを除外します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`exclude.internal.topics`属性参照 <br/>受信対象から`__consumer_offsets`のような内部トピックを除外します。 |
| クライアントID | `dataflow` | string | V1 | Kafka Consumerを識別するIDを入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`client.id`属性参照 |
| パーティション割り当てポリシー | - | string | V1 | Kafkaでメッセージ受信時にコンシューマーグループにどのようにパーティションを割り当てるか決定します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`partition.assignment.strategy`属性参照 <br/>org.apache.kafka.clients.consumer.RangeAssignor<br/>org.apache.kafka.clients.consumer.RoundRobinAssignor<br/>org.apache.kafka.clients.consumer.StickyAssignor<br/>org.apache.kafka.clients.consumer.CooperativeStickyAssignor |
| オフセット設定 | `latest` | enum | V1 | コンシューマーグループのオフセットを設定する基準を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`auto.offset.reset`属性参照 <br/>以下の設定すべて、コンシューマーグループが既に存在する場合は既存のオフセットを維持します。<br/>none: コンシューマーグループがない場合はエラーを返します。<br/>earliest: コンシューマーグループがない場合はパーティションの最も古いオフセットで初期化します。<br/>latest: コンシューマーグループがない場合はパーティションの最新オフセットで初期化します。 |
| オフセットコミット周期 | `5000` | number | V1 | コンシューマーグループのオフセットを更新する周期を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`auto.commit.internal.ms`属性参照 |
| オフセット自動コミット可否 | `true` | boolean | V1 | コンシューマーオフセットを自動で更新するかを決定します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`enable.auto.commit`属性参照 |
| キーデシリアライズタイプ | `org.apache.kafka.common.serialization.StringDeserializer` | string | V1 | 受信するメッセージのキーをシリアライズする方法を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`key.deserializer`属性参照 |
| メッセージデシリアライズタイプ | `org.apache.kafka.common.serialization.StringDeserializer` | string | V1 | 受信するメッセージの値をシリアライズする方法を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`value.deserializer`属性参照 |
| メタデータ生成可否 | `false` | boolean | V1 | 属性値がtrueの場合、メッセージに関するメタデータフィールドを作成します。メタデータフィールドをSinkプラグインに露出するためにはfilterノードタイプを組み合わせる必要があります(下段ガイド参照)。 | 作成されるフィールドは次の通りです。<br/>topic: メッセージを受信したトピック<br/>consumer\_group: メッセージの受信に使用したコンシューマーグループID<br/>partition: メッセージを受信したトピックのパーティション番号<br/>offset: メッセージを受信したパーティションのオフセット<br/>key: メッセージキーを含むByteBuffer |
| Fetch最小サイズ | - | number | V1 | 1回のfetchリクエストで取得するデータの最小サイズを入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`fetch.min.bytes`属性参照 |
| 転送バッファサイズ | - | number | V1 | データ転送に使用するTCP sendバッファのサイズ(byte)を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`send.buffer.bytes`属性参照 |
| 再試行リクエスト周期 | - | number | V1 | 転送リクエストが失敗した場合に再試行する周期(ms)を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`retry.backoff.ms`属性参照 |
| 巡回冗長検査 | `true` | boolean | V1 | メッセージのCRCを検査します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`check.crcs`属性参照 |
| サーバー再接続周期 | - | number | V1 | ブローカーサーバーへの接続が失敗した場合に再試行する周期を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`reconnect.backoff.ms`属性参照 |
| Pollタイムアウト | `100` | number | V1 | トピックから新しいメッセージを取得するリクエストに対するタイムアウト(ms)を入力します。 |  |
| パーティションごとのFetch最大サイズ | - | number | V1 | パーティションごとに1回のfetchリクエストで取得する最大サイズを入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`max.partition.fetch.bytes`属性参照 |
| サーバーリクエストタイムアウト | - | number | V1 | 転送リクエストに対するタイムアウト(ms)を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`request.timeout.ms`属性参照 |
| TCP受信バッファサイズ | - | number | V1 | データの読み取りに使用するTCP receiveバッファのサイズ(byte)を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`receive.buffer.bytes`属性参照 |
| セッションタイムアウト | - | number | V1 | コンシューマーのセッションタイムアウト(ms)を入力します。<br/>コンシューマーが該当時間内にheartbeatを送信できなかった場合、コンシューマーグループから除外します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`session.timeout.ms`属性参照 |
| 最大pollメッセージ数 | - | number | V1 | 1回のpollリクエストで取得する最大メッセージ数を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`max.poll.records`属性参照 |
| 最大poll周期 | - | number | V1 | pollリクエスト間の最大周期(ms)を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`max.poll.interval.ms`属性参照 |
| Fetch最大サイズ | - | number | V1 | 1回のfetchリクエストで取得する最大サイズを入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`fetch.max.bytes`属性参照 |
| Fetch最大待機時間 | - | number | V1 | `Fetch最小サイズ`設定分のデータが集まらない場合、fetchリクエストを送信する待機時間(ms)を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`fetch.max.wait.ms`属性参照 |
| コンシューマーヘルスチェック周期 | - | number | V1 | コンシューマーがheartbeatを送信する周期(ms)を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`heartbeat.interval.ms`属性参照 |
| メタデータ更新周期 | - | number | V1 | パーティション、ブローカーサーバーの状態などを更新する周期(ms)を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/producer-configs/)の`metadata.max.age.ms`属性参照 |
| IDLEタイムアウト | - | number | V1 | データ転送がないコネクションを閉じる待機時間(ms)を入力します。 | [Kafka公式ドキュメント](https://kafka.apache.org/23/configuration/consumer-configs/)の`connections.max.idle.ms`属性参照 |
