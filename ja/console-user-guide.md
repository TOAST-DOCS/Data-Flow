...
3. ノード接続でノードの動作順序を決定してフローを完成させます。
4. フローを実行します。
...
| RUNNING        | フローが実行中です。 |
| ERROR              | フロー実行プロセスで通信障害または認証不可などによりエラーが発生しました。継続的に`ERROR`が発生する場合は **カスタマーサポート > お問い合わせ**よりお問い合わせください。 |
...
* 過去15分のログを確認できます。
* V2エンジンタイプのログは、次の2つのうち1つを選択して確認できます。
...
* 倍率調整、初期化、画面調整、ノードソートにより、フローが画面によく見えるように調整できます。
    * 倍率調整
...
* Sourceノードでスキーマを定義した場合、各ノードの入力/出力スキーマを確認できます。
* フローの全てのSourceノードにスキーマが定義されている場合のみ、スキーマ情報が表示されます。
* フィールドの色でスキーマの変換結果を視覚的に確認できます。
    * 緑色: 追加されたフィールド
    * 青色: タイプが変換されたフィールド
    * 黄色: タイプの競合が検出されたフィールド
* 複数の親ノードで同じフィールド名が異なるタイプとして定義された場合、警告が表示されます。
* Filterノードのプロパティを変更すると、スキーマの変換結果をリアルタイムでプレビューできます。
* スキーマの定義方法と各ノード別の変換の動作については、[ノード設定ガイド](./node-config-guide/)をご参照ください。
...
* 左上の**Cloud Schedulerへ移動**をクリックすると、新しいウィンドウでスケジュールを登録できます。
* Cloud Schedulerを通じて登録されたフローのスケジュール一覧を表示します。
* スケジュール名、対象テンプレート名、実行タイプ、開始日時、終了日時、有効化ステータスを表示します。
* スケジュール名の右側にあるボタンをクリックすると、新しいウィンドウで該当スケジュールの詳細情報を確認できます。
...
* リストの折りたたみでモニタリング詳細ページ領域を拡張できます。
...
* 倍率調整と初期化で、フローが画面によく見えるように調整できます。
    * 倍率調整
        * フローグラフが表示される画面の倍率を調整します。
        * 0.5倍～2倍までの倍率をサポートします。
    * 初期化
...
* タイプの選択
    * V2-ALL
        * イベントIn/Out、ネットワークデータ送受信は、1つのグラフで表示されます。
        * CPU使用率、メモリ使用量は、V2-JOB、V2-TASKグラフをそれぞれ表示します。
    * V2-JOB, V2-TASK
...
* テンプレート設定のノードタイプまたはテンプレートからフローコンポーネントを呼び出してテンプレートを定義します。
...
②ログを保存するLog & Crash Search **Appkey**と保存する**ログレベル**を入力します。
③ **保存**を押して設定を完了します。
